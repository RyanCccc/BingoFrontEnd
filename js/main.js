...
$(function(){
	...
	$("#selection_handle").click(function(){
		...
		if(!$("#selection").is(":hidden") && !isMoving){
			isMoving=true;
			$("#schedule").animate({"width":"+=25%"},500);
			 $("#selection").hide( "slide", {direction:'right'},500,function(){isMoving=false;});
		}
	});
...
function setUp(course){
	...
	if($("#"+coursename).size()==0){
	    ...
	    if($("#selection").is(":hidden") ){
		    $("#schedule").animate({"width":"-=25%"},500);
		    $("#selection").show( "slide", {direction:'right'},500 ,function(){$("#" + coursename).show("slide", {direction:'up'},300);});
	    }else{
		    $("#" + coursename).show("slide", {direction:'up'},300);
	    }
	}else{
...
function createDraggableInSel(tt,coursename,type){
	...
	tt.draggable({ revert: function(socketObj){
				if(socketObj==false){
					// remove conflict
					$(".RED").removeClass("RED");
	...
	tt.on("dragstart",function(){
			...
			var testclass;
			if(type=="lecture"){
				test1=[[["Mon",9,1],["Wed",9,1],["Fri",9,1]],10000];
				test2=[[["Mon",11,1],["Wed",11,1],["Fri",11,1]],10002];
				testclass=[test1,test2];
			}else if(type=="lab"){
				test1=[[["Tu",9,2],["Th",9,2]],10100];
				test2=[[["Tu",13,2],["Th",13,2]],10200];
				testclass=[test1,test2];
			}else if(type=="recitation"){
				test1=[[["Mon",14,1]],10120];
				test2=[[["Fri",14,1]],10110];
				testclass=[test1,test2];
			}
			// End Test
...
function createDropBox(classDetail,part,draggable,index){
	...
	dropBox.css({"top":top+"px","height":height+"px"}).appendTo("."+day);
...
function createDraggableInSch($obj){
	...
	$drag.css({"width":"100%" , "height":"100%"});
	$drag.data("myDropBox",$obj);
	$drag.draggable({ revert: function(socketObj){
				if(socketObj==false){
					$drag.css({"width":"100%" , "height":"100%"});
	...
	$drag.on("dragstart",function(){
			var thisCRN=$obj.attr("value");
			$drag.css({"width":"50px" , "height":"50px"});
...
function enableSelection(){
	if(!isMoving && $("#selection").is(":hidden")){
		$("#selection").show( "slide", {direction:'right'},500);
		$("#schedule").animate({"width":"-=25%"},500);
	}
}
...
function disableSelection(){
	...
	if(!isMoving && !$("#selection").is(":hidden")){
		$("#selection").hide( "slide", {direction:'right'},500);
		$("#schedule").animate({"width":"+=25%"},500);
	}
}